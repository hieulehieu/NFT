...
export default function Table() {
  ...
  const fetchTransactionHistory = async () => {
    try {
      const mintTxs = await getTransactionByEvents(wBNB_ADDRESS, ERC20_ABI, 'Minted');
      const burntTxs = await getTransactionByEvents(wBNB_ADDRESS, ERC20_ABI, 'Burnt');
      ...
      <div className={styles.table}>
        ...
        <div className={styles['table-list-item-body']}>
          {historyTransactions && historyTransactions.length > 0 ? (
            [...historyTransactions, ...historyTransactions].map((item, index) => (
              <div className={styles['table-list']} key={index}>
                <Link to={`${BSC_SCAN}/tx/${item.hash}`} target="_blank" className={styles['table-list-item']}>
                ...
                <div className={styles['table-list-item']}>
                  {formatHexValue(item.amount._hex)} {item.name === 'Minted' ? 'BNB' : 'wBNB'}
                </div>
                <div className={`${styles['table-list-item']} text-center`}>
                  <Icon icon="ep:right" fontSize={16} />
                </div>
                <div className={styles['table-list-item']}>
                  {formatHexValue(item.amount._hex)} {item.name === 'Minted' ? 'wBNB' : 'BNB'}
                </div>
