...
import { LOAN_ADDRESS, CHAIN_ID, wBNB_ADDRESS, RPC_URL } from '@src/constants';
...
export const generateOrderSignature = async (order) => {
  const encodedOrder = ethers.utils.solidityPack(
    ...
    [order.creator, order.nftAddress, order.nftTokenId, order.offer, order.duration, order.repayment, order.lender]
  );
...
export const getTransactionByEvents = async (address, abi, eventName) => {
  try {
    ...
    const events = await contract.queryFilter(eventName, toBlock - 100000, toBlock);
    return events;
  } catch (error) {
    return [];
  }
...
export const generateRequestSignature = async (
  ...
  const { loanId, loanDuration, renegotiateFee, repayment } = requestData;
  const { signer: signerAddress, nonce, expiry } = signatureData;
  ...
  const payload = ethers.utils.solidityPack(
    ...
    [loanId, loanDuration, repayment, renegotiateFee, encodedSignature, loanContract, chainId]
  );
