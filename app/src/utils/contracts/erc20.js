...
import { wBNB_ADDRESS, LOAN_ADDRESS } from '@src/constants';
...
export const getRawBalance = async (account, contractAddress = wBNB_ADDRESS) => {
  const contract = ERC20Contract(contractAddress, provider);
  return contract.balanceOf(account);
};

export const getBalance = async (account, contractAddress = wBNB_ADDRESS) => {
  const contract = ERC20Contract(contractAddress, provider);
  const balance = await contract.balanceOf(account);
  return Number(ethers.utils.formatEther(balance)).toFixed(2);
};

export const getContractSymbol = async (contractAddress = wBNB_ADDRESS) => {
  const contract = ERC20Contract(contractAddress, provider);
  return contract.symbol();
};

export const checkAllowance = async (owner, amount, spender = LOAN_ADDRESS, contractAddress = wBNB_ADDRESS) => {
  const contract = ERC20Contract(contractAddress, provider);
...
export const mintERC20 = async (amount, contractAddress = wBNB_ADDRESS) => {
  const signer = provider.getSigner();
  const contract = ERC20Contract(contractAddress, signer);
...
export const burnERC20 = async (amount, contractAddress = wBNB_ADDRESS) => {
  const signer = provider.getSigner();
  const contract = ERC20Contract(contractAddress, signer);
  const decimals = await contract.decimals();
